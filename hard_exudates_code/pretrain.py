...
net.load_parameters('pretrain.params',ctx=ctx)
net.hybridize()
...
sw = SummaryWriter(logdir='./logs_pretrain', flush_secs=2)
...
def train(train_data, net, loss, ctx ,global_step,epoch_step, num_epochs,best_F1=0):
    ...
    for epoch in range(num_epochs):
        if epoch<50:
            ...
            trainer = gluon.Trainer(net.collect_params(),'adam', {'learning_rate': 0.000001, 'wd':1e-3})
        elif epoch<120:
            trainer = gluon.Trainer(net.collect_params(),'adam', {'learning_rate': 0.0000001, 'wd':1e-3})
        else:
            trainer = gluon.Trainer(net.collect_params(),'sgd', {'learning_rate': 0.00000001,'momentum': 0.9,'wd':1e-3})
        ...
        if F1>best_F1:
            net.save_parameters('pretrain.params')
            best_F1=F1
        if epoch == 0:
            sw.add_graph(net)
