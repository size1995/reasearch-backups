...
class UnetSkipUnit(HybridBlock):
    def __init__(self, inner_channels, outer_channels, inner_block=None, innermost=False, outermost=False,
        ...
        with self.name_scope():
            ...
            if use_attention:
                self.attention=attention_block.attention_block(x_channel=outer_channels,y_channel=outer_channels)
            if innermost:
    ...
    def hybrid_forward(self, F, x):
        if self.outermost:
            ...
            if self.use_attention:
                return F.concat(self.model(x),self.attention(x,self.model(x)))
            else:
