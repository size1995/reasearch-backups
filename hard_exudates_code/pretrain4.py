...
sw = SummaryWriter(logdir='./logs_pretrain4', flush_secs=2)
...
def train(train_data, net, loss, ctx ,global_step,epoch_step, num_epochs,best_F1=0):
    ...
    for epoch in range(num_epochs):
        ...
        acc_list,tpr_list,tnr_list,ppv_list,ACC,TPR,TNR,PPV,F1=tool.evaluate(img_path,label_path,net,crop_size=256,stride=128,ctx=ctx[0],threshold=0.5,enhance=True)
        ...
        if F1>best_F1:
            net.save_parameters('pretrain4.params')
            best_F1=F1
        if epoch == 0:
            sw.add_graph(net)
...
"""
train(train_data, net, loss, ctx ,global_step,epoch_step, num_epochs=80,best_F1=0.70)
"""