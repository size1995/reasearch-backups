...
sw = SummaryWriter(logdir='./logs_pretrain_dice', flush_secs=2)
...
def train(train_data, net, loss, ctx ,global_step,epoch_step, num_epochs,best_F1=0):
    ...
    for epoch in range(num_epochs):
        ...
        if F1>best_F1:
            net.save_parameters('pretrain1.params')
            best_F1=F1
        if epoch == 0:
            sw.add_graph(net)
...
train(train_data, net, loss, ctx ,global_step,epoch_step, num_epochs=80,best_F1=0)
